...
function displayStations(stations)
{
	...
	for (let i = 0; i < size; i++)
	{
		...
		input.value = `station${i + 1}:${label.innerHTML}`;
...
function addPhoto()
{
  let label = document.getElementById('add_photo');

  if (photos_count > PHOTOS_MAX)
    return;

  let file = document.getElementById(`add_photo${photos_count}`).files[0];
  let reader = new FileReader();
  reader.onloadend = function() {
    document.getElementById(`photo${photos_count++}`).style.backgroundImage = `url(${reader.result})`;
    label.htmlFor = `add_photo${photos_count}`;
  }
...
{
  let label = document.getElementById('add_photo');
  label.htmlFor = 'add_photo1';

  for (let i = 1; i <= photos_count; i++)
  {
    document.getElementById(`photo${i}`).style.backgroundImage = null;
    document.getElementById(`add_photo${i}`).value = '';
  }
...
document.querySelectorAll('.photos-change>input').forEach(function(element) {element.addEventListener('change', addPhoto)});
